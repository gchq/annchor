...
class Annchor:
    ...
    def get_sample(self):
        ...
        self.sample_ixs = self.sampler.sample(
            self.features, self.feature_names, self.n_samples, self.not_computed_mask
        )
        ...
        if self.low_cpu:
            ...
            self.sample_y = np.array(
                [self.f(self.X[ij[0]], self.X[ij[1]]) for ij in self.sample_ijs]
            )
    ...
    def fit_predict_regression(self):
        ...
        self.regression.fit(self.sample_features, self.feature_names, self.sample_y)
    ...
    def select_refine_candidate_pairs(self, w=0.5):
        ...
        self.candidates = self.candidates = np.argpartition(-prob, max_refine)[
            :max_refine
        ]
    ...
    def fit(self):
        ...
        start = time.time()
        if self.verbose:
            print("selecting/refining candidate pairs (1)")
        ...
        start = time.time()
        if self.verbose:
            print("selecting/refining candidate pairs (2)")
        self.select_refine_candidate_pairs()
        if self.verbose:
            timeit("select_refine_candidate_pairs", origin, start)
