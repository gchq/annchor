...
class Annchor:
    ...
    def get_sample(self):
        ...
        for nbin in range(self.n_sample_bins):
            ...
            samples[nbin] = np.random.choice(
                indices[mask], size=(bin_size + (nbin < remainder))
            )

        self.sample_ixs = np.hstack([samples[i] for i in range(self.n_sample_bins)])

        self.sample_y = self.get_exact_ijs(
            self.f, self.X, self.features[self.sample_ixs, :2].astype(int)
        )
    ...
    def get_approx(self):
        ...
        non_anchors = np.array(
            [ix for ix in set(np.arange(self.nx)).difference(set(self.A))]
        )
        ...
        if self.low_cpu:
            ...
            for k in range(n):
                i, j = np.random.choice(
                    non_anchors, replace=False, size=2
            ...
            sample_exact = self.get_exact_ijs(self.f, self.X, self.sample_IJs)
        ...
        self.LRApprox = self.LR.predict(
            self.Bounds.reshape(self.nx ** 2, 2), self.F.flatten()
    ...
    def adjust_p(self):
        ...
        def get_t_from_p(p):
            ...
            lower = self.LRApprox[slc] + hs[self.cluster_map[slc]] * (
                self.diff[slc] != 0
            )
    ...
    def refine_approx(self):
        ...
        """
        if len(self.IJs) > 0:
            self.RefineApprox = self.get_RA(
                self.f, self.X, np.array(self.IJs), self.RefineApprox
            )
    ...
    def refine_part_two(self):
        ...
        for it in range(self.refine_cycles):
            ...
            self.RefineApprox = self.get_RA(
                self.f, self.X, np.array(IJs), self.RefineApprox
            )
    ...
    def fit(self):
        ...
        if self.verbose:
            print("get_error_distributions:", time.time() - start)

        if self.verbose:
            print("refining approximation...")
        self.refine_approx()
        if self.verbose:
            print("refine_approx:", time.time() - start)

        if self.verbose:
            print("computing second pass refinement")
    ...
    def get_sample_lm(self):
        ...
        IJs = np.array(
            [np.random.choice(non_anchors, replace=False, size=2) for k in range(n)]
        )
    ...
    def get_lr_partitions_lm(self):
        ...
        self.dts = (
            [0] + [diff[ixs[i * ng // splits]] for i in range(1, splits)] + [np.infty]
        )

        # Calculate the error distributions in each partition
        self.hs = []
        for ix in [
            ixs[i * ng // splits : (i + 1) * ng // splits] for i in range(splits)
        ]:
            # ix = np.argsort(G[:, 2])[: n // 3]
            r = (
                np.clip(self.LR.predict(self.G[ix, :2]), self.G[ix, 0], self.G[ix, 1])
                - self.G[ix, 2]
            )
    ...
    def get_neighbor_graph(self):
        ...
        if "numba" in str(type(self.f)):
            ...
            def get_exact(f, X, IJ):
                ...
                return np.array(
                    Parallel(n_jobs=CPU_COUNT)(delayed(_f)(ij) for ij in IJ)
                )
